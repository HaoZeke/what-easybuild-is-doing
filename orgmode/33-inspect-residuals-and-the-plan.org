...
#+end_prerequisites
[[file:12-eb-stack-updater-and-generator.org][eb-stack ports a recipe and bumps one]] placed the conda-forge recipe beside the =.eb= and halted at the residual queue.
...
#+end_example
The =locks/= directory is missing and the =easyconfigs/= directory is missing.
...
The sections that matter on a first read are:
...
Written paths are identities (=conda-forge/recipe.yaml=, =spack/package.py=).
...
Each residual includes =id=, =stage=, =category=, =severity=, =summary=.
Each residual may also contain =evidence= and =provenance=.
#+begin_example
{
...
#+end_example
Severity has three values that constitute the triage.
...
2. Which file is added, and what does a =preferred= pin in it control? 
3. After a successful plan, which two paths must exist before =campaign run= starts?
#+begin_solution
