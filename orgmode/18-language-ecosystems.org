...
It precedes the specifics.
The language's tool may perform the build.
It must not perform dependency resolution.
* Python
...
=pip= runs; it is not permitted to work out what else it needs.
...
Identical structure with distinct identifiers.
