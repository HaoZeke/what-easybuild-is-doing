...
=SYSTEM= on line seven is not a keyword.
=SYSTEM= on line seven is not a string.
It is a value EasyBuild put in the namespace before evaluating the file: ={'name': 'system', 'version': 'system'}=.
It has no quotes around it while =moduleclass= does.
One is a variable.
...
Look at the last two lines of that output.
The file names no easyblock.
The model reports the one EasyBuild will derive from the name anyway.
That is the rest of this chapter.
...
The convention against it is a good one.
It is a *convention*.
...
It uses a dictionary that EasyBuild assembles from the easyconfig.
...
It resolves.
That something is the easyblock.
It is the subject of the remainder of this chapter.
* EasyBuild derives the easyblock from the name, and the derivation is a string transformation
The easyconfig above does not state how to install anything.
...
The module searched for is =code_server=, and the class inside it is specified next.
...
Class names have to be valid Python identifiers and package names are not, so something has to bridge the two.
The bridge also has to be *reversible*: without that, two differently-named packages could collide on one class.
...
It computes =mapped_arch=, the source of that template value.
It raises an error for any architecture that is neither x86-64 nor aarch64.
It defaults the install step to copying the unpacked tree into place.
It runs =code-server --help= as a sanity check.
The install is not finished until the binary has run.
...
The parameters =error_on_missing_easyblock= and =error_on_failed_import= both default to true.
...
All material in this chapter addresses the inferences generated by EasyBuild.
An incorrect inference from an actual run on an actual cluster is shown.
The package is QMCPACK, a quantum Monte Carlo code, and the easyconfig was shifted from a pinned development commit to the 4.4.0 release.
...
It is the only unread line in the sample.
It causes more confusion than the other nineteen lines combined.
